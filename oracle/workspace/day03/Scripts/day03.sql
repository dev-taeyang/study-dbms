...
--SELECT TEAM_ID , HEIGHT, PLAYER_NAME  FROM PLAYER
...
--SELECT PLAYER_NAME, NVL(NICKNAME, '지욱') FROM PLAYER
--WHERE NICKNAME IS NULL;
...
--SELECT PLAYER_NAME, NVL("POSITION", '정지욱') "GK" FROM PLAYER
...
--SELECT PLAYER_NAME, NATION , NVL2(NATION, '등록', '미등록') AS "등록여부" FROM PLAYER;
...
--SELECT "POSITION" , AVG(HEIGHT) "포지션별 평균키",
--(
--	SELECT AVG(HEIGHT) "전체 평균 키" FROM PLAYER 
--)
--FROM PLAYER
--WHERE "POSITION" IS NOT NULL 
--GROUP BY "POSITION";
...
/*PLAYER 테이블에서 송종국 선수가 속한 팀의 전화번호 검색하기   INNER조인이 디폴트 값이다.  */
--SELECT TEAM_ID FROM PLAYER
--WHERE PLAYER_NAME = '송종국';
--
--SELECT TEL FROM TEAM
--WHERE TEAM_ID = 'K06';
--
--SELECT T.TEL "팀 전화번호" FROM PLAYER P JOIN TEAM T
...
SELECT J.JOB_TITLE , E.EMAIL , E.LAST_NAME , E.FIRST_NAME  FROM JOBS j JOIN EMPLOYEES e
ON J.JOB_ID = E.JOB_ID;

/*EMP 테이블의 SAL을 SALGRADE 테이블의 등급으로 나누기*/
SELECT E.SAL "연봉", S.GRADE "등급" ,S.LOSAL "저연봉", S.HISAL "고연봉" 
...
SELECT * FROM EMPLOYEES;
SELECT E.EMPLOYEE_ID "사원 넘버" ,E.FIRST_NAME "성", E.LAST_NAME "이름" , D.DEPARTMENT_NAME "부서명", E.HIRE_DATE "고용일"
FROM EMPLOYEES e JOIN DEPARTMENTS d
ON E.DEPARTMENT_ID = D.DEPARTMENT_ID
WHERE E.HIRE_DATE BETWEEN '2003-01-01' AND '2005-12-31';

/*JOB_TITLE 중 'Manager'라는 문자열이 포함된 직업들의 평균 연봉을 JOB_TITLE별로 검색 %Manager  */

SELECT J.JOB_TITLE "직종", AVG(E.SALARY)  
FROM JOBS j JOIN EMPLOYEES e
ON J.JOB_ID  = E.JOB_ID AND J.JOB_TITLE LIKE '%Manager%'
GROUP BY J.JOB_TITLE;

/*EMP 테이블에서 ENAME에 L이 있는 사원들의 DNAME과 LOC 검색*/
SELECT E.ENAME, D.DNAME , D.LOC  FROM
EMP e JOIN DEPT d
ON e.DEPTNO  = d.DEPTNO
AND e.ENAME LIKE '%L%'
GROUP BY D.DNAME , D.LOC, E.ENAME;
/*축구 선수들 중에서 각 팀별로 키가 가장 큰 선수들 전체 정보 검색*/
SELECT T.TEAM_ID, MAX(P.HEIGHT) 
